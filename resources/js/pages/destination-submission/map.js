document.addEventListener('DOMContentLoaded', function() {
    ...
    function initMap() {
        ...
        // Setup pencarian dengan autocomplete (pastikan pencarian juga dibatasi ke Indonesia jika perlu)
    ...
    function setupSearchWithAutocomplete() {
        ...
        searchInput.addEventListener('input', function() {
            ...
            debounceTimer = setTimeout(() => {
                ...
                // Ambil hasil dari Nominatim
                fetch(
                        `https://nominatim.openstreetmap.org/search?format=json&countrycodes=id&q=${encodeURIComponent(query)}&limit=5`
                    )
                        ...
                        if (data && data.length > 0) {
                            ...
                            data.forEach(result => {
                                ...
                                item.addEventListener('click', function() {
                                    ...
                                    // Update input dengan lokasi terpilih
        ...
        searchInput.addEventListener('keypress', function(e) {
            if (e.key === 'Enter') {
                ...
                if (query.length > 2) {
                    fetch(
                            `https://nominatim.openstreetmap.org/search?format=json&countrycodes=id&q=${encodeURIComponent(query)}&limit=1`
                        )
                        .then(response => response.json())
                        .then(data => {
                            if (data && data.length > 0) {
